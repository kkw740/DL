...
# Train function
def train(lr, bs, layers, hidden, epochs=5):
    model = nn.Sequential(nn.Flatten(), *[layer for i in range(layers) for layer in [nn.Linear(784 if i==0 else hidden, hidden), nn.ReLU()]], nn.Linear(hidden, 10))
    ...
    for _ in range(epochs):
        ...
        model.eval()
        with torch.no_grad(): acc = (model(X_val).argmax(1)==y_val).float().mean().item()
        acc_hist.append(acc)
    return acc_hist, acc

# Configs: [LR, Batch, Layers, Hidden, Name]
configs = [(0.001,32,2,128,'Baseline'), (0.01,32,2,128,'High LR'), (0.001,64,2,128,'Large Batch'), (0.001,32,3,128,'Deep'), (0.001,32,2,64,'Small')]
results = []

print("Hyperparameter Tuning Results\n" + "="*50)
for lr, bs, layers, hidden, name in configs:
    hist, acc = train(lr, bs, layers, hidden)
    results.append((name, lr, bs, layers, hidden, acc, hist))
    print(f"{name:12} | LR={lr:5}, BS={bs:2}, L={layers}, H={hidden:3} | Acc={acc:.4f}")

# Visualize
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 4))
for name, *_, hist in results: ax1.plot(hist, marker='o', label=name)
ax1.set_xlabel('Epoch'); ax1.set_ylabel('Val Accuracy'); ax1.set_title('Training Curves'); ax1.legend(); ax1.grid(True)
names, accs = [r[0] for r in results], [r[5] for r in results]
ax2.bar(names, accs, color=['blue','red','green','orange','purple']); ax2.set_ylabel('Final Accuracy'); ax2.set_title('Comparison'); ax2.grid(axis='y')
...
print(f"4. Hidden: 128={results[0][5]:.3f} vs 64={results[4][5]:.3f} → More units better")
best = max(results, key=lambda x: x[5])
print(f"\nBest: {best[0]} with {best[5]:.4f} accuracy")
